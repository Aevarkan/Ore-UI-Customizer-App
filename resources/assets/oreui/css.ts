...
class cssjs<TestMode extends false | ((...args: [string, string]) => any) = false> {
    ...
    parseRules(rules: string): CSSJSRule[] {
        ...
        for (var i = 0; i < rulesList.length; i++) {
            var line: string | string[] = rulesList[i]!;
    ...
    compressCSS(cssObjectArray: CSSJSObject[]): CSSJSObject[] {
        ...
        for (var i: number = 0; i < cssObjectArray.length; i++) {
            ...
            var found: CSSJSObject[] = this.findBySelector(cssObjectArray, obj.selector); //found compressed
            if (found.length !== 0) {
    ...
    intelligentCSSPush(cssObjectArray: CSSJSObject[], minimalObject: CSSJSObject, reverse?: boolean | undefined): void {
        ...
        if (reverse === false) {
            ...
            for (var j: number = cssObjectArray.length - 1; j > -1; j--) {
        ...
        if (cssObject === false) {
            ...
            if (minimalObject.type !== "media" && minimalObject.rules && cssObject.rules) {
                for (var ii: number = 0; ii < minimalObject.rules.length; ii++) {
